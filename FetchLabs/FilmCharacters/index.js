...
function getCharacters(url, film, characters){
  film.innerHTML='';
  characters.innerHTML='';
  let movie=getIt(url);
  let promise = movie.next().value;
  promise.then((value)=>{
    return value.json();
  }).then((value)=>{
    film.innerHTML=value.title;
    return value.characters;
  }).then((value)=>{
    value.forEach((characterUrl)=>{
      let character=getIt(characterUrl);
      let promiseCharacter = character.next().value;
      promiseCharacter.then((value)=>{
        return value.json();
      }).then((value)=>{
        characters.innerHTML=characters.innerHTML + `${value.name}<br>`;
      })
    });
  })
}
...
document.getElementById("idBtn").onclick = ()=> {
  const filmID = document.getElementById("idFilmNumber").value;
  const url = `https://swapi.co/api/films/${filmID}/`;
  const characters = getCharacters(url, document.getElementById("idFilmName"), document.getElementById("idCharacters"));
};
